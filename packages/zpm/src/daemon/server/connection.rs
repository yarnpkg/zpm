...
pub async fn handle_connection(
    ...
    // Track active subscriptions for this connection (RAII cleanup on drop)
    let mut _subscription_guards: Vec<SubscriptionGuard> = Vec::new();
    // Notification receivers for all active subscriptions
    let mut notification_receivers: Vec<mpsc::UnboundedReceiver<DaemonNotification>> = Vec::new();

    loop {
        // Build a future that resolves when any notification receiver has data
        let notification_future = poll_notifications(&mut notification_receivers);

        tokio::select! {
            biased;

            // Prioritize incoming messages over notifications
            msg_opt = read.next() => {
                ...
                match msg {
                    Message::Text(text) => {
                        ...
                        // Create subscription BEFORE dispatch if this is a PushTasks request
                        ...
                        {
                            // Only create subscription if at least one scope is not None
                            if *output_subscription != SubscriptionScope::None
                                || *status_subscription != SubscriptionScope::None
                            {
                                let (sub_id, rx) = ctx.subscription_registry.create_subscription(
                                    *output_subscription,
                                    *status_subscription,
                                    context_id.clone(),
                                );
                        ...
                        // Dispatch handles adding tasks AND updating subscription atomically
                    ...
                    Message::Close(frame) => {
                        // Send close response before breaking
            ...
            Some(notification) = notification_future => {
                // No filtering needed - registry already filtered for us
    ...
    // _subscription_guards dropped here, automatically cleaning up subscriptions
    Ok(())
}

/// Poll all notification receivers and return the first available notification
async fn poll_notifications(
    receivers: &mut [mpsc::UnboundedReceiver<DaemonNotification>],
) -> Option<DaemonNotification> {
    if receivers.is_empty() {
        // No subscriptions yet, wait forever (other branch will win)
            ...
            for rx in receivers.iter_mut() {
                match rx.poll_recv(cx) {
                    ...
                    std::task::Poll::Ready(None) => {
                        // Channel closed, continue checking others
                    }
