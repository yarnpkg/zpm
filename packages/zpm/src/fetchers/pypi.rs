...
async fn resolve_artifact_url(context: &InstallContext<'_>, params: &PypiRegistryReference) -> Result<String, Error> {
    ...
    let bytes = project.http_client.cached_get(&metadata_url).await?;
    let metadata: PypiVersionMetadata = JsonDocument::hydrate_from_slice(&bytes[..])?;

    let wheel = select_best_wheel(&metadata.urls)
...
pub async fn fetch_locator<'a>(context: &InstallContext<'a>, locator: &Locator, params: &PypiRegistryReference, is_mock_request: bool) -> Result<FetchResult, Error> {
    ...
    let artifact_url = resolve_artifact_url(context, params).await?;

    let cached_blob = package_cache.ensure_blob(locator.clone(), ".zip", || async {
        let response = project.http_client.get(&artifact_url)?
            .send()
            .await?;
