...
// packages/zpm-constraints/index.ts
var index_exports = {};
...
// packages/zpm-constraints/constraintsUtils.ts
var import_toPath = __toESM(require_toPath());

// packages/zpm-constraints/miscUtils.ts
...
// packages/zpm-constraints/nodeUtils.ts
...
// packages/zpm-constraints/index.ts
var manifestUpdates = /* @__PURE__ */ new Map();
var userWorkspaceErrors = /* @__PURE__ */ new Map();
...
var createSetFn = (workspaceCwd) => (path, value, { caller = getCaller() } = {}) => {
  ...
  const workspaceUpdates = getMapWithDefault(manifestUpdates, workspaceCwd);
  const pathUpdates = getFactoryWithDefault(workspaceUpdates, key, () => ({
  ...
  for (const [workspaceCwd, workspaceUpdates] of manifestUpdates) {
    ...
    for (const { fieldPath, values } of workspaceUpdates.values()) {
      if (values.size > 1) {
        ...
        const currentValue = (0, import_lodash.get)(manifest, fieldPath);
