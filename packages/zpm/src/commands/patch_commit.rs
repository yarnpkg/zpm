...
///
/// With `-s,--save`, the patch file is written to the folder configured by `patchFolder`, and the top-level manifest is updated with a `patch:`
/// resolution that points to it.
///
...
pub struct PatchCommit {
    ...
    /// Path to the edited folder created by `yarn patch`
    source: Path,
}
