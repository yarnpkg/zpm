use crate::{error::Error, Path, Value};
...
impl Parser {
    ...
    fn parse_object(&mut self) -> Result<(Value, Span), Error> {
        ...
        if let Some(token) = self.current_token() {
            if matches!(token.token, Token::RightBrace) {
                ...
                self.advance();
                return Ok((Value::Object(entries), end_span));
            }
        }
    ...
    fn parse_array(&mut self) -> Result<(Value, Span), Error> {
        ...
        if let Some(token) = self.current_token() {
            if matches!(token.token, Token::RightBracket) {
                let end_span = token.span;
                self.advance();
                return Ok((Value::Array(elements), end_span));
            }
        }
...
impl JsonFormatter {
    ...
    pub fn set<P: Into<Path>>(&mut self, path: P, value: Value) -> Result<(), Error> {
    ...
    pub fn update<P: Into<Path>>(&mut self, path: P, value: Value) -> Result<(), Error> {
    ...
    pub fn remove<P: Into<Path>>(&mut self, path: P) -> Result<(), Error> {
        self.remove_path(&path.into())
    }

    pub fn remove_path(&mut self, path: &Path) -> Result<(), Error> {
        self.set_path(path, Value::Undefined)
    }
    ...
    fn serialize_value(&self, value: &Value, depth: usize) -> String {
        match value {
            Value::Null => {
                "null".to_string()
            },
...
fn set_at_path(current: &mut Value, path: &[String], value: Value, create_if_missing: bool) -> Result<bool, Error> {
    if path.is_empty() {
        *current = value;
        return Ok(false);
    }
    ...
    match current {
        Value::Object(entries) => {
            ...
            for i in 0..entries.len() {
                if entries[i].0 == *key {
                    if remaining_path.len() > 0 || value != Value::Undefined {
            ...
            // Key not found
            if value == Value::Undefined || !create_if_missing {
                return Ok(false);
            }
        ...
        Value::Array(elements) => {
            if let Ok(index) = key.parse::<usize>() {
                if remaining_path.is_empty() && value == Value::Undefined {
...
mod tests {
    ...
    fn test_adding_nested_object() {
        ...
        formatter.set(
            ...
            Value::Object(vec![
                ...
                ("notifications".to_string(), Value::Bool(true)),
            ])
        ).unwrap();
    ...
    fn test_deeply_nested_creation() {
        ...
        formatter.set(
            ...
            Value::String("deep value".to_string())
        ).unwrap();
    ...
    fn test_mixed_array_object_creation() {
        ...
        formatter.set(
            ...
            Value::Array(vec![
                ...
                Value::Object(vec![
                    ...
                    ("age".to_string(), Value::Number("25".to_string())),
                ]),
            ])
        ).unwrap();
        ...
        formatter.set(
            ...
            Value::Object(vec![
                ...
                ("options".to_string(), Value::Array(vec![
                    Value::String("opt1".to_string()),
                    Value::String("opt2".to_string()),
                ])),
            ])
        ).unwrap();
    ...
    fn test_empty_containers() {
        ...
        formatter.set(["empty_object"], Value::Object(vec![])).unwrap();
        formatter.set(["empty_array"], Value::Array(vec![])).unwrap();
    ...
    fn test_array_index_creation() {
        ...
        formatter.set(["data", "0"], Value::String("first".to_string())).unwrap();
        formatter.set(["data", "2"], Value::String("third".to_string())).unwrap();
        formatter.set(["data", "1"], Value::String("second".to_string())).unwrap();
    ...
    fn test_json_path_basic_dot_notation() {
        let path = Path::from_file_string("foo.bar.baz").unwrap();
    ...
    fn test_json_path_array_notation() {
        let path = Path::from_file_string("foo[0]").unwrap();
        assert_eq!(path.segments(), &["foo", "0"]);
        assert_eq!(path.to_file_string(), "foo[0]");

        let path = Path::from_file_string("foo[123]").unwrap();
    ...
    fn test_json_path_string_bracket_notation() {
        let path = Path::from_file_string(r#"foo["bar"]"#).unwrap();
    ...
    fn test_json_path_complex() {
        let path = Path::from_file_string(r#"users[0].name"#).unwrap();
        assert_eq!(path.segments(), &["users", "0", "name"]);
        assert_eq!(path.to_file_string(), "users[0].name");

        let path = Path::from_file_string(r#"data["key-with-dash"][0].value"#).unwrap();
    ...
    fn test_json_path_special_chars() {
        ...
        let path = Path::from_file_string(r#"["key with spaces"]"#).unwrap();
        assert_eq!(path.segments(), &["key with spaces"]);
        assert_eq!(path.to_file_string(), r#"["key with spaces"]"#);

        let path = Path::from_file_string(r#"["key.with.dots"]"#).unwrap();
        ...
        let path = Path::from_file_string(r#"["123"]"#).unwrap();
    ...
    fn test_json_path_escapes() {
        let path = Path::from_file_string(r#"["key\"with\"quotes"]"#).unwrap();
        assert_eq!(path.segments(), &[r#"key"with"quotes"#]);
        assert_eq!(path.to_file_string(), r#"["key\"with\"quotes"]"#);

        let path = Path::from_file_string(r#"["key\\with\\backslashes"]"#).unwrap();
    ...
    fn test_json_path_empty() {
        let path = Path::from_file_string("").unwrap();
    ...
    fn test_json_path_errors() {
        ...
        assert!(Path::from_file_string("foo[").is_err());
        assert!(Path::from_file_string("foo[0").is_err());
        assert!(Path::from_file_string(r#"foo["bar"#).is_err());

        // Invalid bracket content
        assert!(Path::from_file_string("foo[]").is_err());
        assert!(Path::from_file_string("foo[bar]").is_err()); // Unquoted string
    }
    ...
    fn test_json_formatter_with_json_path() {
        ...
        let path = Path::from_file_string("users[0].name").unwrap();
        formatter.set_path(&path, Value::String("Bob".to_string())).unwrap();
        let path = Path::from_file_string("users[1].age").unwrap();
        formatter.set_path(&path, Value::Number("25".to_string())).unwrap();
    ...
    fn test_json_formatter_bracket_notation_keys() {
        ...
        let path = Path::from_file_string(r#"["key with spaces"]"#).unwrap();
        formatter.set_path(&path, Value::String("value1".to_string())).unwrap();
    ...
    fn test_json_path_roundtrip() {
        ...
        for test_case in test_cases {
            let path = Path::from_file_string(test_case).unwrap();
            let segments = path.segments().to_vec();
            let reconstructed = Path::from_segments(segments.clone());
    ...
    fn test_remove_object_property() {
        ...
        // Remove a property
        formatter.set(["value"], Value::Undefined).unwrap();
    ...
    fn test_remove_nested_property() {
        ...
        formatter.set(["user", "email"], Value::Undefined).unwrap();
    ...
    fn test_remove_array_element() {
        ...
        formatter.set(["items", "2"], Value::Undefined).unwrap();
    ...
    fn test_remove_multiple_elements() {
        ...
        formatter.set(["b"], Value::Undefined).unwrap();
        formatter.set(["d"], Value::Undefined).unwrap();
    ...
    fn test_remove_last_element() {
        ...
        formatter.set(["only"], Value::Undefined).unwrap();
    ...
    fn test_remove_from_complex_structure() {
        ...
        let path = Path::from_file_string("users[1]").unwrap();
        formatter.set_path(&path, Value::Undefined).unwrap();
        // Remove a setting
        formatter.set(["settings", "timeout"], Value::Undefined).unwrap();
    ...
    fn test_undefined_in_objects() {
        ...
        formatter.set(
            ...
            Value::Object(vec![
                ...
                ("c".to_string(), Value::Number("3".to_string())),
                ("d".to_string(), Value::Undefined),
            ])
        ).unwrap();
    ...
    fn test_json_path_from_vec() {
        ...
        let path: Path = ["foo", "bar", "baz"].into();
        assert_eq!(path.segments(), &["foo", "bar", "baz"]);

        // Test From<Vec<&str>>
        let path: Path = ["users", "0", "name"].into();
        assert_eq!(path.segments(), &["users", "0", "name"]);

        // Test empty vec
        let path: Path = Vec::<String>::new().into();
        assert!(path.is_empty());

        // Test mixed usage
        let segments = ["api", "v1", "endpoints"];
        let path: Path = segments.into();
