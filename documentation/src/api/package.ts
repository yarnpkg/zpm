...
export function useReleaseInfo({name, version}: {name?: string, version?: string}): ReleaseJsDelivrInfo {
  return useSuspenseQuery({
    queryKey: [`packageFiles`, name, version],
    queryFn: async () => {
      const req = await fetch(`https://data.jsdelivr.com/v1/package/npm/${name}@${version}/flat`);
      const res = await req.json();
      ...
      return {
        files: res.files as Array<ReleaseFile>,
        fileSet,
      };
    },
  }).data!;
...
export type UseReleaseFileParams = {
  name: string;
  version: string;
  path?: string;
};

export function useReleaseFile({name, version, path}: UseReleaseFileParams) {
  return useSuspenseQuery({
...
export type UseReleaseReadmeParams = {
  name: string;
  readme?: string;
  version: string;
  versions: any;
...
export function useReleaseReadme({name, readme, version, versions}: UseReleaseReadmeParams) {
  const releaseInfo = useReleaseInfo({name, version});

  const readmeFile = versions?.[version]?.npm?.readme
    ?? releaseInfo.files.find((entry: ReleaseFile) => entry.name.toLowerCase() === `/readme.md`);
  ...
  domPurify.addHook(`uponSanitizeAttribute`, (node, data) => {
    if (data.attrName === `src` && !data.attrValue.startsWith(`//`) && !data.attrValue.includes(`:`)) {
      ...
      if (releaseInfo.files.some(entry => entry.name === url)) {
        ...
        const normalizedRepositoryUrl = normalizeRepoUrl(versions?.[version].repository?.url);
...
export type UseResolutionParams = {
  name: string;
  version: string;
  versionData?: any;
};

export type UseResolutionOptions = {
  mainFields: Array<keyof ReleaseNpmInfo>;
  conditions: Array<string>;
};

export function useResolution(
  {name, version, versionData}: UseResolutionParams,
  {mainFields, conditions}: UseResolutionOptions,
) {
  ...
  if (versionData?.npm?.exports && !exportsResolution)
    return null;
  ...
  for (const mainField of mainFields) {
    const resolution = resolveQualifier(releaseInfo, versionData?.npm?.[mainField] || `.`);
    if (resolution !== null) {
      return resolution;
    }
  }
...
export function useWeeklyDownloads(name: string) {
  return useSuspenseQuery({
    ...
    queryFn: async () => {
      const req = await fetch(`https://api.npmjs.org/downloads/range/last-week/${name}`);
      const res = await req.json();
