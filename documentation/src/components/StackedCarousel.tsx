...
import LeftChevron from "src/assets/svg/left-chevron.svg?react";
...
function getSlideStatus(index: number, current: number, total: number) {
  ...
  if (diff === 2) return total === 4 ? "background-next" : "background-next";
  if (diff === -2) return total === 4 ? "background-next" : "background-prev";
