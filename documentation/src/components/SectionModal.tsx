...
function isHeadingWrapper(element: HTMLElement | null): boolean {
  ...
  if (!element.classList.contains("sl-heading-wrapper")) return false;
...
function collectSectionContentFromHeadingWrapper(
  ...
  while (current) {
    if (current !== startWrapper && isHeadingWrapper(current)) break;
...
function findSectionFromHash(): string | null {
  ...
  if (!startWrapper) {
    const allWrappers = Array.from(
      document.querySelectorAll(".sl-heading-wrapper")
    ).filter((el) => isHeadingWrapper(el as HTMLElement)) as HTMLElement[];
    if (allWrappers.length === 0) return null;

    const preceding = allWrappers.filter(
      (h) =>
        (h.compareDocumentPosition(target) &
